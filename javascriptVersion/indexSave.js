...
let b = a.add(new Vec3(2, 1, 0.5)).add(new Vec3(2, 1, 0.5)).div(4)
console.log(b)
...
const randomInUnitSphere = () => {
  while (true) {
    let p = new Vec3(- 1 + 2 * Math.random(), - 1 + 2 * Math.random(), - 1 + 2 * Math.random())
...
class Ray {
  ...
  rayColor = (world, depth) => {
    ...
    if (hit) {
      ...
      this.origin = returnedRecord.p
      this.direction = target.sub(returnedRecord.p)
      return this.rayColor(world, depth - 1).mul(0.5)
    }
  ...
  for (let j = IMAGE_HEIGHT - 1; j >= 0; j--) {
    ...
    for (let i = 0; i < IMAGE_WIDTH; i++) {
      ...
      for (let s = 0; s < SAMPLE_PER_PIXEL; s++) {
        ...
        pixelColor.addThis(r.rayColor(world, MAX_DEPTH))
      }
